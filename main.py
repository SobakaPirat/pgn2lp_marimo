...
def _(mo):
    mo.md("""# Convert pgn file to liquipedia format""")
    return
...
def _(mo):
    ...
    mo.md(f"""### Upload your pgn file here\n{pgn_file}""").callout()
    return (pgn_file,)


@app.cell
def _(StringIO, codecs, mo, parse_pgn, pgn_file):
    try:
        pgn_decoded = StringIO(
            codecs.decode(pgn_file.value[0].contents, encoding="utf-8")
        )
        games_info = parse_pgn(pgn_decoded)
    except Exception as e:
        games_info = ""
        if pgn_file.value:
            mo.output.append(mo.md(f"""Unable to read your file:\n{e}"""))
    return (games_info,)
...
def _(mo):
    ...
    round_selector = mo.ui.number(start=0, step=1)
    ...
    round_tab = mo.vstack(["Round to filter", round_selector])
...
@app.cell
def _(games_info, generate_matches, mo):
    matches = generate_matches(games_info)
    copy_result = mo.ui.code_editor(value=matches, max_height=300)

    mo.accordion({"Matches for Round robbin or Swiss bracket": copy_result})
    return
...
def _(games_info, mo):
    ...
    copy_participants = mo.ui.code_editor(value=participant_table, max_height=300)

    mo.accordion({"Participant Table": copy_participants})
    return
...
def _(draw_points, loss_points, points_changed, re, read_game, win_points):
    ...
    def parse_pgn(pgn):
        ...
        while True:
            ...
            game_info["white_formated"] = swap_name_surname(
                re.sub("\\b[A-Z]{3}\\b", "", game_info["white"])
            )
            game_info["white_team"] = game.headers.get("WhiteTeam", "")
            game_info["black"] = game.headers.get("Black", "")
            game_info["black_formated"] = swap_name_surname(
                re.sub("\\b[A-Z]{3}\\b", "", game_info["black"])
            )
            game_info["black_team"] = game.headers.get("BlackTeam", "")
            game_info["date"] = game.headers.get(
                "UTCDate", ""
            ) or game.headers.get("Date", "")
            game_info["date_formated"] = re.sub(
                "-\\?\\?|\\?{4}", "", game_info["date"].replace(".", "-")
            )
            game_info["time"] = game.headers.get(
                "UTCTime", ""
            ) or game.headers.get("Time", "")
            game_info["time_formated"] = (
                f" - {game_info['time']}" if game_info.get("time") else ""
            )
            game_info["result"] = game.headers.get("Result", "")
            game_info["result_formated"] = format_result(game_info["result"])
            game_info["round"] = (
                game.headers.get("Round", "").replace(",", ".").split(".")[0]
            )
            game_info["eco"] = game.headers.get("ECO", "")
            game_info["moves"] = game.end().board().fullmove_number
            if points_changed.value:
                game_info["white_score"] = custom_score(
                    game_info["result_formated"], True
                )
                game_info["black_score"] = custom_score(
                    game_info["result_formated"], False
                )
...
@app.cell(hide_code=True)
def _(players_to_filter, round_to_filter):
    def generate_matches(games):
        output_text = ""
        for game in games:
            # filter players
            if players_to_filter and all(
                ...
                continue
            # filter round
            if (round_to_filter == 0) or (str(round_to_filter) == game["round"]):
                output_text += f"""|{{{{Match
        |finished=true
        |date={game["date_formated"]}{f"{game['time_formated']} {{{{abbr/UTC}}}}" if game.get("time_formated") else ""}
        |opponent1={{{{1Opponent|flag=|{game["white_formated"]}{f"|score={game['white_score']}" if game.get("white_score") else ""}}}}}
        |opponent2={{{{1Opponent|flag=|{game["black_formated"]}{f"|score={game['black_score']}" if game.get("black_score") else ""}}}}}
        |map1={{{{Map|winner={game["result_formated"]}|white=1|eco={game["eco"]}|length={game["moves"]}}}}}
    }}}}
    """
        return output_text
...
def generate_participant_table(games):
    ...
    for participant_name, participant_team in participants.items():
        participants_output.append(
            f"|{{{{1Opponent|{participant_name}|flag={participant_team}|team=}}}}\n"
        )
